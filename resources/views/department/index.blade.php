...
@csrf
<h1 style="text-align:center; margin-top:2%;">Departments Record</h1>
...
<table class="table table-hover">
  ...
  @foreach ($departments as $department)
    ...
    '{{ $department->name }}', 
    '{{ $department->location }}')">View</button>

  <button class="btn btn-primary" type="button" onclick="openEditModal('{{ $department->id }}', 
            '{{ $department->name }}', 
            '{{ $department->location }}')">Update</button>

  <button class="btn btn-danger" type="button" onclick="openDeleteModal({{ $department->id }})">Delete</button>
</td>
    </tr>
    @endforeach
  </tbody>
