<script>
 function openEditModal(id, name = '', phone = '',cnic = '', department = '', date_of_birth = '', address = '', gender = '') {
    // Set form action
    document.getElementById('editForm').action = `/student/${id}`;
...
<div class="edit-model">
 <div class="modal fade" id="editModal" tabindex="-1" aria-labelledby="editModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    ...
    @enderror</label>
  <input type="text" id="editName" class="form-control" name="name" value="{{ $student->name }}">
  @error('name')
  ...
  @error('cnic')
    <span class="text-danger">*</span>
  @enderror</label>
  <input type="text" id="editCNIC" class="form-control" name="cnic" value="{{ $student->cnic }}">
  @error('cnic')
            <div class="error text-danger">{{ $message }}</div>
        @enderror
        </div>
  ...
  @error('phone')
    <span class="text-danger">*</span>
  @enderror</label>
  <input type="text" id="editPhone" class="form-control" name="phone" value="{{ $student->phone }}">
  @error('phone')
            <div class="error text-danger">{{ $message }}</div>
        @enderror
        </div>
  ...
  @error('date_of_birth')
    <span class="text-danger">*</span>
  @enderror</label>
  <input type="date" id="editDateOfBirth" class="form-control" name="date_of_birth" value="{{ $student->date_of_birth }}">
  @error('date_of_birth')
            <div class="error text-danger">{{ $message }}</div>
        @enderror
        </div>
  ...
  @error('address')
    <span class="text-danger">*</span>
  @enderror</label>
  <input type="text" id="editAddress" class="form-control" name="address" value="{{ $student->address }}">@error('address')
            <div class="error text-danger">{{ $message }}</div>
        @enderror
  ...
  <label for="name">Gender:&nbsp&nbsp
   @error('gender')
            ...
            @error('gender')
              <div class="error text-danger">{{ $message }}</div>
            @enderror</label>
    <input class="form-check-input" type="radio" name="gender" id="editMale" value="male" {{ $student->gender == 'male' ? 'checked' : '' }}>
    <label class="form-check-label" for="editMale">Male</label>
    &nbsp
    <input class="form-check-input" type="radio" name="gender" id="editFemale" value="female" {{ $student->gender == 'female' ? 'checked' : '' }}>
